...
# Filepath for storing the JSON data
...
def load_data():
    global habits, target_days, current_day_index, habit_data, start_date
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, "r") as file:
            ...
            habit_data = {int(k): v for k, v in data.get("habit_data", {}).items()}
            start_date = datetime.strptime(data.get("start_date", datetime.today().strftime("%Y-%m-%d")), "%Y-%m-%d").date()
load_data()
...
class HabitTrackerApp(CTk):
    def __init__(self):
        ...
        if os.path.exists(DATA_FILE):
            with open(DATA_FILE, "r") as file:
                try:
                    data = json.load(file)
                    if data.get("habits") and data.get("target_days") > 0:
                        self.create_tracking_page()
                    else:
                        self.create_main_page()
                except json.JSONDecodeError:
                    self.create_main_page()  # Start with the main page if data is corrupted
        else:
            self.create_main_page()
    ...
    def confirm_habits_and_target(self):
        ...
        try:
            ...
            save_data()  # Save target days and start date
            self.create_tracking_page()
        except ValueError:
    ...
    def perform_analysis(self):
        try:
            ...
            # Calculate the maximum days we have data for
            max_days_available = len(habit_data)

            # If days_to_analyze exceeds max_days_available, limit it
            if days_to_analyze > max_days_available:
                days_to_analyze = max_days_available
                messagebox.showinfo("Info", f"Analyzing data for the last {max_days_available} days (available data).")

            if days_to_analyze <= 0 or max_days_available == 0:
                messagebox.showerror("Error", "No data available to analyze or invalid input.")
                return

            # Count habit completion over specified days (starting from the most recent day)
            habit_counts = {habit: 0 for habit in habits}
            for day in range(current_day_index - days_to_analyze, current_day_index):
                if day in habit_data:
                    for habit, completed in habit_data[day].items():
                        if completed:
                            habit_counts[habit] += 1

            if not any(habit_counts.values()):
                messagebox.showinfo("Analysis", "No habits completed in the selected period.")
                return

            # Display graph for habit completion
            plt.figure(figsize=(8, 4))
            plt.bar(habit_counts.keys(), habit_counts.values(), color="skyblue")
